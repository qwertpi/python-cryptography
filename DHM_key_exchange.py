...
if ALICE:
	...
	p = generate_p()
	g = generate_g()
	tcp_send.send(Raw(f"{p};{g}"))
	
	personal_secret = generate_personal_secret()
	tcp_send.send(modular_exponenate(g, personal_secret, p))
	
	bobs_public_value = int(tcp_recv.sniff(count=1))
...
if BOB:
	...
	p, g = map(int, tcp_recv.sniff(count=1).split(";"))
	personal_secret = generate_personal_secret()
	tcp_send.send(modular_exponenate(g, personal_secret, p))
	alices_public_value = int(tcp_recv.sniff(count=1))
